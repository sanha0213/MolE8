...
Reads in .xyz files and predicts energies of the molecules using the trained neural 
...
NN_EModelDistName = 'NN_Energy_Model_dist.h5'
NN_GModelDistName = 'NN_FreeEnergy_Model_dist.h5'
KRR_EModelDistName = 'KRR_Energy_Model_dist.plk'
KRR_GModelDistName = 'KRR_FreeEnergy_Model_dist.plk'
...
parser.add_argument("trained_option", help='choose between d (trained with distorted molecule replicates) or wd (trained without distorted molecule replicates)', action='store')
...
trained_option_correct = ['d', 'wd']

Filename = args.filename
logging.basicConfig(filename='./MolE8_v2_'+args.model_option+'_'+args.energy_option+'_'+args.trained_option+'_'+Filename[:-4]+'.log', level=logging.DEBUG, format='%(message)s', filemode='w')

if args.model_option not in model_option_correct:
    print('model_option not entered correctly. Use NN for neural networks, KRR for kernel ridge regression and MLR for multivariable linear regression machine learning models')
    logging.info('model_option not entered correctly. Use NN for neural networks, KRR for kernel ridge regression and MLR for multivariable linear regression machine learning models')
    sys.exit()

if args.energy_option not in energy_option_correct:
    print('energy_option not entered correclty, use E for energy prediction and G for free energy prediction')
    logging.info('energy_option not entered correclty, use E for energy prediction and G for free energy prediction')
    sys.exit()


Model_Option = args.model_option
Energy_Option = args.energy_option
Trained_Option = args.trained_option


if args.trained_option not in trained_option_correct:
    print('trained_option not entered correclty, use d for model trained with distorted molecule replicates and wd for model trained without distorted molecule replicates')
    logging.info('trained_option not entered correclty, use d for model trained with distorted molecule replicates and wd for model trained without distorted molecule replicates')
    sys.exit()
...
X_MatrixOutput = open('./MolE8_v2_ReadableXMatrix_'+Filename[:-4]+'_'+Model_Option+'_'+Energy_Option+'_'+Trained_Option+'.txt', 'w')
...
readableXfile = open('./MolE8_v2_ReadableXFeatures_'+Filename[:-4]+'_'+Model_Option+'_'+Energy_Option+'_'+Trained_Option+'.txt', 'w')
...
np.save('./MolE8_v2_X'+'_'+Filename[:-4]+'_'+Model_Option+'_'+Energy_Option+'_'+Trained_Option, X, allow_pickle=True)
...
if Model_Option == 'NN' and Energy_Option == 'E':
    ...
    if Trained_Option == 'wd':
        ...
        batch_size = 64
        X_trainstd = 41.10011507592822
        X_scaled = X/X_trainstd
        
        model = load_model('./'+database_loc+'/'+NN_EModelDistName)
        model.summary()
    ...
    if Trained_Option == 'wd':
        ...
        batch_size = 64
        X_trainstd = 40.40963781203339
        X_scaled = X/X_trainstd
        
        model = load_model('./'+database_loc+'/'+NN_GModelDistName)
        model.summary()
    ...
    if Trained_Option == 'wd':
        ...
        kernel = pickle.load(open('./'+database_loc+'/'+KRR_EModelDistName, 'rb'))
        unwrapped_predictions = kernel.predict(X)
    ...
    if Trained_Option == 'wd':
        ...
        kernel = pickle.load(open('./'+database_loc+'/'+KRR_GModelDistName, 'rb'))
        unwrapped_predictions = kernel.predict(X)
